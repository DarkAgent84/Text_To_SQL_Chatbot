...
Usage Examples:
    ...
    python data_profiler.py --file data/sample.csv

    # 2. Generate LLM Prompt Markdown schema
    python data_profiler.py --file data/sample.xlsx --format markdown

    # 3. Export JSON schema dictionary to file
    python data_profiler.py --file data/sample.csv --to-json schema_dict.json

    # 4. Generate clean SQL CREATE TABLE DDL
    python data_profiler.py --file data/sample.csv --format ddl

    # 5. Ingest file into SQLite database with properly typed columns
    python data_profiler.py --file data/sample.csv --to-sqlite app.db --table-name loans
"""
...
# Add project root to sys.path so app.core.data_profiler can be imported directly
